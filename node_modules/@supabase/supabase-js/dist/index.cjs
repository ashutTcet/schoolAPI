...
const createClient = (supabaseUrl, supabaseKey, options) => {
	const hasValue = (value) => typeof value === "string" ? value.trim().length > 0 : value !== null && value !== void 0;
	const resolveFromEnv = (env, names) => {
	...
	const readEnv = () => {
		...
		const hasSupabaseVars = hasValue(env.SUPABASE_URL) || hasValue(env.NEXT_PUBLIC_SUPABASE_URL) || hasValue(env.SUPABASE_KEY) || hasValue(env.SUPABASE_ANON_KEY) || hasValue(env.NEXT_PUBLIC_SUPABASE_ANON_KEY);
		if (!hasSupabaseVars) {
			try {
	...
	const env = readEnv();
	const resolvedSupabaseUrl = hasValue(supabaseUrl) ? supabaseUrl : resolveFromEnv(env, ["SUPABASE_URL", "NEXT_PUBLIC_SUPABASE_URL", "VITE_SUPABASE_URL", "REACT_APP_SUPABASE_URL", "supabaseUrl"]);
	const resolvedSupabaseKey = hasValue(supabaseKey) ? supabaseKey : resolveFromEnv(env, ["SUPABASE_KEY", "SUPABASE_ANON_KEY", "NEXT_PUBLIC_SUPABASE_ANON_KEY", "NEXT_PUBLIC_SUPABASE_KEY", "VITE_SUPABASE_ANON_KEY", "VITE_SUPABASE_KEY", "REACT_APP_SUPABASE_ANON_KEY", "REACT_APP_SUPABASE_KEY", "supabaseKey"]);
	if (!hasValue(resolvedSupabaseUrl) || !hasValue(resolvedSupabaseKey)) {
		throw new Error("Missing Supabase configuration. Provide createClient(url, key) explicitly, or set env vars: SUPABASE_URL and SUPABASE_ANON_KEY (or NEXT_PUBLIC/VITE/REACT_APP equivalents).");
	}
	return new SupabaseClient(resolvedSupabaseUrl, resolvedSupabaseKey, options);
};
